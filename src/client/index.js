...
export class XarClient {
   ...
   * @return {Promise} resolves with http response
   */
  async getOracleAssets() {
    try {
      const data = await this._httpClient.request("get", `${api.getOracleAssets}`)
      return data
    } catch (err) {
      return err
    }
  ...
  async getStakingPool() {
    try {
      const data = await this._httpClient.request("get", `${api.getStakingPool}`)
      return data
    } catch (err) {
      return err
    }
